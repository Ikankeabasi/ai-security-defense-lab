...
def render_level4(user, supabase_client):
    ...
    st.markdown(
        ...
        '<thead><tr style="background:#F1F5F9;"><th style="padding:10px 14px;text-align:left;color:#64748B;border-bottom:1px solid #E2E8F0;width:28%;">STRIDE Category</th><th style="padding:10px 14px;text-align:left;color:#64748B;border-bottom:1px solid #E2E8F0;">My Finding</th></tr></thead>' +
        '<tbody>' +
        '<tr style="border-bottom:1px solid #F1F5F9;"><td style="padding:10px 14px;color:#374151;font-weight:500;">Spoofing</td><td style="padding:10px 14px;color:#6B7280;line-height:1.6;">The RAG layer accepts the &#96;tenant_id&#96; directly from the request and does not verify it against the authenticated session. In &#96;rag_config.py&#96;, &#96;TRUST_CLIENT_TENANT_ID = True&#96; and &#96;VERIFY_TENANT_SESSION = False&#96;. I confirmed the weakness in the lab by using a Meridian session with a different tenant ID and retrieving that tenant&#39;s records. The fix is to derive the tenant from the authenticated session and reject any mismatched client-supplied value.</td></tr><tr style="border-bottom:1px solid #F1F5F9;"><td style="padding:10px 14px;color:#374151;font-weight:500;">Tampering</td><td style="padding:10px 14px;color:#6B7280;line-height:1.6;">The fine-tuning pipeline accepts training data without checking its integrity before it reaches the model. In &#96;finetune_pipeline_dag.py&#96;, &#96;VALIDATE_DATA_INTEGRITY = False&#96;, and the DAG passes data from the shared source directly to the fine-tuning step. The lab&#39;s poisoned-data demonstration shows how a malicious batch can introduce hidden model behaviour. Training data should be validated and approved before it reaches the fine-tuning stage.</td></tr><tr style="border-bottom:1px solid #F1F5F9;"><td style="padding:10px 14px;color:#374151;font-weight:500;">Repudiation</td><td style="padding:10px 14px;color:#6B7280;line-height:1.6;">The fine-tuning pipeline does not require source approval before training data is used. &#96;REQUIRE_SOURCE_SIGNOFF = False&#96; in &#96;finetune_pipeline_dag.py&#96;, while &#96;pull_training_data()&#96; is described as pulling files directly from the shared source without filtering or validation. This weakens accountability for what entered a training run. A source approval step and an auditable record of training-data changes should be enforced before training.</td></tr><tr style="border-bottom:1px solid #F1F5F9;"><td style="padding:10px 14px;color:#374151;font-weight:500;">Information Disclosure</td><td style="padding:10px 14px;color:#6B7280;line-height:1.6;">The PayGuard vector store uses one shared index, &#96;payguard-shared-index&#96;, with &#96;METADATA_FILTER_ENFORCED = False&#96;. The lab demonstration showed cross-tenant retrieval, and the test suite confirms that a Meridian session can retrieve Alpine documents when the tenant boundary is bypassed. The same level also demonstrates that a leaked raw embedding can expose information from a source record. Tenant isolation must therefore be enforced at the vector-database layer, with raw vector access restricted.</td></tr><tr style="border-bottom:1px solid #F1F5F9;"><td style="padding:10px 14px;color:#374151;font-weight:500;">Denial of Service</td><td style="padding:10px 14px;color:#6B7280;line-height:1.6;">The retrieval layer has no query limit: &#96;RATE_LIMIT_ENABLED = False&#96; and &#96;MAX_QUERIES_PER_MINUTE = None&#96;. The scale demonstration shows that repeated requests can probe the shared index across multiple tenant IDs without throttling. This can increase inference/compute cost and affect legitimate users. Rate limiting and per-user or per-tenant query limits should be enabled at the retrieval layer.</td></tr><tr style="border-bottom:1px solid #F1F5F9;"><td style="padding:10px 14px;color:#374151;font-weight:500;">Elevation of Privilege</td><td style="padding:10px 14px;color:#6B7280;line-height:1.6;">A client can change the tenant value used by the retrieval request and cross the intended tenant boundary. The combination of &#96;TRUST_CLIENT_TENANT_ID = True&#96; and &#96;METADATA_FILTER_ENFORCED = False&#96; allows the authenticated Meridian session in the lab to retrieve another client&#39;s documents. The database/vector-store layer should enforce the authenticated tenant independently of the request value so changing &#96;tenant_id&#96; cannot grant access to another tenant.</td></tr><tr style="border-bottom:1px solid #F1F5F9;"><td style="padding:10px 14px;color:#374151;font-weight:500;">OWASP Classification</td><td style="padding:10px 14px;color:#6B7280;line-height:1.6;">The tenant-isolation, shared-vector-store and retrieval weaknesses are classified under **OWASP LLM09:2026 — Vector and Embedding Weaknesses**. The fine-tuning data-integrity weakness is classified under **OWASP LLM05:2026 — Data and Model Poisoning**. The Semgrep rules also identify **CWE-639** for the user-controlled tenant key, **CWE-284** for improper access control, **CWE-345** for insufficient data-authenticity verification, and **CWE-770** for missing resource limits.</td></tr><tr style="border-bottom:1px solid #F1F5F9;"><td style="padding:10px 14px;color:#374151;font-weight:500;">Business Impact</td><td style="padding:10px 14px;color:#6B7280;line-height:1.6;">The weaknesses create a realistic path to cross-tenant exposure of confidential financial records, including portfolio, estate-planning and advisory information shown in the lab. The fine-tuning weakness can also place hidden behaviour into the model, while unrestricted retrieval can increase compute costs. For a FinTech RAG system, the combined effect is a loss of tenant confidentiality, trust, and control over AI behaviour.</td></tr><tr style="border-bottom:1px solid #F1F5F9;"><td style="padding:10px 14px;color:#374151;font-weight:500;">Root Cause</td><td style="padding:10px 14px;color:#6B7280;line-height:1.6;">The common pattern is that important security boundaries are trusted rather than independently enforced. The RAG system trusts a client-supplied tenant identity and a shared vector store without a database-level tenant boundary; the fine-tuning pipeline trusts its training-data source without integrity validation or sign-off; and the retrieval layer has no resource limit.</td></tr><tr><td style="padding:10px 14px;color:#374151;font-weight:500;">Remediation</td><td style="padding:10px 14px;color:#6B7280;line-height:1.6;">Move tenant enforcement into the database/vector-store layer and derive the tenant from the authenticated session. Set &#96;METADATA_FILTER_ENFORCED = True&#96;, validate and approve training data before fine-tuning, enable retrieval rate limiting, and protect raw embeddings. After patching, run &#96;python3 tests/test_payguard_rag.py&#96; and confirm that the cross-tenant, scale, fine-tuning-validation, and backdoor tests are blocked before submitting the evidence.</td></tr>' +
        '</tbody></table>' +
